...
int possuiValor(Chave* chavesFolha, int valor){
    int possui = 0; // False
    while(chavesFolha != NULL){
        if (chavesFolha->valor == valor){
            possui = 1;
        }

        chavesFolha = chavesFolha->prox;
    }

    return possui;
}

Chave* exibirValores(int valorChave, int quantidadeExibir, arvorebm* tree){
    if (tree == NULL || tree->raiz == NULL){
        return NULL;
    }

    noB* folhaComValor = localiza_folha(valorChave, tree);
    if (folhaComValor == NULL){
        printf("\nO valor %i nao foi encontrado!", valorChave);
        return NULL;
    }

    int folhaPossuiValor = possuiValor(folhaComValor->listaChaves->inicio, valorChave);
    if (folhaPossuiValor == 0){
        printf("\nO valor %i nao foi encontrado!", valorChave);
        return NULL;
    }

    int qtdValoresMostrados = 0;

    Chave* chaveAux = NULL;
    Chave* chaveResposta = NULL;
    while (folhaComValor != NULL){
        chaveAux = folhaComValor->listaChaves->inicio;
        while (chaveAux != NULL){
            if (chaveAux->valor >= valorChave){
                printf("%i, ", chaveAux->valor);
                qtdValoresMostrados++;
            }

            if (chaveAux != NULL){
                chaveResposta = chaveAux;
            }

            chaveAux = chaveAux->prox;

            if (qtdValoresMostrados == quantidadeExibir){
                return chaveResposta;
            }
        }

        folhaComValor = folhaComValor->proximo;
    }
